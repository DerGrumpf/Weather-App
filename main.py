...
def get_api_key():
    apikeys = ['392d7379ca75bb85b522dc52abeafe14', 'd186a5e3e7fb7c3cbebfa8755a0df37f',
        '4393174048690a8abd59d3cba7c3388c', 'af28c3e112fef89cf2f77a0052745a96',
        'e8096bad73124d731d7cb2c1a1871c94', '383dacd23d13795e25fdde25726ba823']
    index = random.randint(0, len(apikeys)-1)
...
class AddLocationForm(ModalView):
    ...
    def search_location(self):
    	...
    	search_url=search_template.format(self.search_input.text, get_api_key())
...
class CurrentWeather(BoxLayout):
    location = ListProperty(['Brunswick', 'DE'])
    ...
    def update_weather(self):
        ...
        weather_url = weather_template.format(self.location[0], self.location[1], temp_type, get_api_key())
...
class Forecast(BoxLayout):
    ...
    def update_weather(self):
        config = WeatherApp.get_running_app().config
        temp_type = config.getdefault("General", "temp_type", "metric").lower()
        weather_template = "http://api.openweathermap.org/data/2.5/forecast/daily?q={},{}&units={}&cnt=3&APPID={}"
        weather_url = weather_template.format(self.location[0], self.location[1], temp_type, get_api_key())
        request = UrlRequest(weather_url, self.weather_retrieved)
